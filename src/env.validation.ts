...
export const validationSchema = Joi.object({
    ...
    AI_DISABLED: Joi.string().valid('true', 'false').default('false'),
    LLM_NORMALIZE_LIMIT: Joi.number().integer().min(0).max(50).default(1),
    LLM_TIMEOUT_MS: Joi.number().integer().min(1000).default(120000),
    LLM_MIN_INTERVAL_MS: Joi.number().integer().min(0).default(1200),
    LLM_MAX_RETRIES: Joi.number().integer().min(0).max(10).default(2),
    LLM_RETRY_BASE_MS: Joi.number().integer().min(100).default(1500),
    LLM_DEBUG_RAW: Joi.string().valid('true', 'false').default('false'),
